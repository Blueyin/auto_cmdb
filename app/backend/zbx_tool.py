...
class zabbix_api:
    ...
    def user_login(self):
	data = json.dumps(
	    {"jsonrpc": "2.0", "method": "user.login", "params": {"user": "blue", "password": "blue1234"}, "id": 0})
	...
	data = json.dumps(
	    {"jsonrpc": "2.0", "method": "host.get", "params": {"output": ["hostid", "name"], "groupids": groupids, },
	     "auth": self.user_login(), "id": 1})
	request = urllib2.Request(self.url, data)
	...
	data = json.dumps({"jsonrpc": "2.0", "method": "host.get",
			   "params": {"output": ["hostid", "name"], "templateids": templateids, },
			   "auth": self.user_login(), "id": 1})
	request = urllib2.Request(self.url, data)
	request.add_header("Content-Type", "application/json")
	req = urllib2.urlopen(request)
	...
	data = json.dumps(
	    {"jsonrpc": "2.0", "method": "host.get", "params": {"output": "extend", "filter": {"hostid": hostId}},
	     "auth": self.user_login(), "id": 1})
	request = urllib2.Request(self.url, data)
	for key in self.header:
	    request.add_header(key, self.header[key])
	...
	data = json.dumps({"jsonrpc": "2.0", "method": "hostinterface.get",
			   "params": {"output": "extend", "filter": {"hostid": hostId}}, "auth": self.user_login(),
			   "id": 1})
	request = urllib2.Request(self.url, data)
	for key in self.header:
	    request.add_header(key, self.header[key])
	...
	data = json.dumps(
	    {"jsonrpc": "2.0", "method": "host.get", "params": {"output": "extend", "filter": {"host": hostName}},
	     "auth": self.user_login(), "id": 1})
	request = urllib2.Request(self.url, data)
	for key in self.header:
	    request.add_header(key, self.header[key])
	...
	data = json.dumps(
	    {"jsonrpc": "2.0", "method": "hostinterface.get", "params": {"output": "extend", "filter": {"ip": hostIp}},
	     "auth": self.user_login(), "id": 1})
	request = urllib2.Request(self.url, data)
	for key in self.header:
	    request.add_header(key, self.header[key])
	...
	try:
		...
		if len(hostip) == 0:
		    print "HostID : %s\t HostName : %s\t HostIp : %s\t Status :\33[32m%s\33[0m \t Available :\33[31m%s\33[0m" % (
			hostip['hostid'], host['name'], hostip['ip'], host['status'], host['available'])
		else:
		    print "HostID : %s\t HostName : %s\t HostIp : %s\t Status :\33[32m%s\33[0m \t Available :\33[31m%s\33[0m" % (
			hostip['hostid'], host['name'], hostip['ip'], host['status'], host['available'])
	...
	data = json.dumps({"jsonrpc": "2.0", "method": "hostgroup.get",
			   "params": {"output": "extend", "filter": {"name": hostgroupName, "host": hostName, }},
			   "auth": self.user_login(), "id": 1, })
	...
	data = json.dumps({"jsonrpc": "2.0", "method": "template.get",
			   "params": {"output": "extend", "filter": {"name": templateName}}, "auth": self.user_login(),
			   "id": 1, })
    ...
    def get_trigle(self):
	data = json.dumps({"jsonrpc": "2.0", "method": "trigger.get",
			   "params": {"only_true": '1', "skipDependent": '1', "monitored": '1', "active": '1',
				      "output": "extend", "selectHosts": "['host']"}, "auth": self.user_login(),
			   "id": 1})
	ret = list()
	request = urllib2.Request(self.url, data)
	for key in self.header:
	...
	data = json.dumps({"jsonrpc": "2.0", "method": "hostgroup.create", "params": {"name": hostgroupName},
			   "auth": self.user_login(), "id": 1})
	request = urllib2.Request(self.url, data)
	...
	data = json.dumps({"jsonrpc": "2.0", "method": "host.create", "params": {"host": hostName, "interfaces": [
	    {"type": 1, "main": 1, "useip": 1, "ip": hostIp, "dns": "", "port": "10050"}], "groups": group_list,
	...
	data = json.dumps(
	    {"jsonrpc": "2.0", "method": "host.update", "params": {"hostid": self.host_get(hostip), "status": 1},
	     "auth": self.user_login(), "id": 1})
	request = urllib2.Request(self.url, data)
	for key in self.header:
	    request.add_header(key, self.header[key])
	...
	data = json.dumps(
	    {"jsonrpc": "2.0", "method": "host.update", "params": {"hostid": self.host_get(hostip), "status": 0},
	     "auth": self.user_login(), "id": 1})
	request = urllib2.Request(self.url, data)
	for key in self.header:
	    request.add_header(key, self.header[key])
	...
	data = json.dumps(
	    {"jsonrpc": "2.0", "method": "host.delete", "params": hostid_list, "auth": self.user_login(), "id": 1})
	...
	for i in host_id:
	    host = {"hostid": i}
	    hosts.append(host)
	data = json.dumps({"jsonrpc": "2.0", "method": "template.massadd",
			   "params": {"templates": [{"templateid": template_id}], "hosts": hosts},
			   "auth": self.user_login(), "id": 1})
	request = urllib2.Request(self.url, data)
	...
	hosts = list()
	data = json.dumps({"jsonrpc": "2.0", "method": "template.massremove",
			   "params": {"templateids": template_id, "hostids": host_id}, "auth": self.user_login(),
			   "id": 1})
	request = urllib2.Request(self.url, data)
	...
	data = json.dumps({"jsonrpc": "2.0", "method": "host.massremove",
			   "params": {"hostids": host_id, "templateids_clear": template_id}, "auth": self.user_login(),
			   "id": 1})
	...
	params = {"output": "extend", "selectGroups": "extend", "selectHosts": "extend", "selectTimeperiods": "extend"}

	data = json.dumps(
	    {"jsonrpc": "2.0", "method": "maintenance.get", "params": params, "auth": self.user_login(), "id": 1})
	...
	data = json.dumps(
	    {"jsonrpc": "2.0", "method": "maintenance.delete", "params": [maintenanceid], "auth": self.user_login(),
	     "id": 1})
	request = urllib2.Request(self.url, data)
	for key in self.header:
	    request.add_header(key, self.header[key])
	...
	params = {"name": maint_name, "active_since": now_time, "active_till": new_main_time, "hostids": hostids,
		  "timeperiods": [{"timeperiod_type": 0, "period": period}]}

	data = json.dumps(
	    {"jsonrpc": "2.0", "method": "maintenance.create", "params": params, "auth": self.user_login(), "id": 1})
