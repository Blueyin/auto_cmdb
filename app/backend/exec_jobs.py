...
def update_operate_sql(jid, ret):
    ...
    cursor = conn.cursor()
    set_names = '''set names utf8'''
    sql = "UPDATE app_schedule_running_list SET  status='%s',end_time='%s',sum_time='%s',result='%s',command='%s' WHERE jid='%s'" % (
	ret[0], ret[1], ret[2], ret[3], ret[4], jid)
    cursor.execute(set_names)
    try:
	cursor.execute(sql)
	cursor.execute("COMMIT")
...
def insert_hostlist_sql(params):
    conn = MySQLdb.connect(host="127.0.0.1", user="test", passwd="test_1234", db="cmdb", charset="utf8")
    cursor = conn.cursor()
    set_names = '''set names utf8'''
    sql = '''INSERT INTO `app_hostlist`
                (`hostname`, `ip`, `bianhao`, `idc_name`,`application`)
                VALUES ( %s, %s, %s, %s, %s)'''
    cursor.execute(set_names)
    # try:
...
def insert_operate_sql(jid, ret):
    ...
    cursor = conn.cursor()
    set_names = '''set names utf8'''
    sql = "UPDATE app_running_list SET  status='%s',end_time='%s',sum_time='%s',result='%s',command='%s' WHERE jid='%s'" % (
	ret[0], ret[1], ret[2], ret[3], ret[4], jid)
    cursor.execute(set_names)
    try:
	cursor.execute(sql)
...
def select_schedule_sql(jid, time_now, action, command_names):
    import datetime
    conn = MySQLdb.connect(host="127.0.0.1", user="test", passwd="test_1234", db="cmdb", charset="utf8")
    cursor = conn.cursor()
    set_names = '''set names utf8'''
...
def insert_schedule_sql(scheduleName, results, time_now):
    ...
    set_names = '''set names utf8'''
    sql = "UPDATE app_running_list SET  end_time='%s',sum_time='%s',status='%s' WHERE name='%s'" % (
	results[1], sum_time, results[0], scheduleName)
    cursor.execute(set_names)
    try:
	cursor.execute(sql)
	cursor.execute("COMMIT")
