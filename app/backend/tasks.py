...
jenkins_server_url="http://192.168.0.8:8080"
host="192.168.0.8"
user_id = "test"
api_token = "****************"
...
def goto_html(full_name,jobName):
    ...
    results.extend([jobName,host,workspace,start_time,"",next_build_number,next_build_number,"执行开始","result",result_log])
    return results
...
def update_operate_running(ret,jid):
    ...
    return True
@task
def select_schedule_running(jid,time_now,action):
    from app.backend.exec_jobs import select_schedule_sql
    sel = select_schedule_sql(jid,time_now,action)
    return sel
@task
...
def get_job_runinng_in(full_name,ret):
    ...
    results.extend(
        [jobName, host, workspace, start_time, "", next_build_number, next_build_number, "执行中", "result",
         result_log])
...
def get_job_running(full_name,jobName):
    ...
    result_log += "\n {0} {1} 程序构建中".format(time_now,full_name)
    results = []
    results.extend([jobName,host,workspace,start_time,"",next_build_number,next_build_number,"执行中","result",result_log])
    #print results
    import_goto_html(results)
    while True:
        builds = server.get_running_builds()
...
def work_running(jobName,hostName,script,action,editor,params):
    ...
    statu = '执行中'
    update_goto_running(job_name,job_id,statu)
    results,status = select_schedule_running(job_id,time_now,action)
...
def schedule_running(scheduleName,user,fun,action,id,all_result):
    ...
    for all_work in all_result:
        job_id,results,status = schedule_for_operate_running(all_work['name'],all_work['host'],all_work['script'],all_work['action'],all_work['editor'],scheduleName,all_work['list_id'],schedule_id)
        if status =="执行失败":
	    break
    if status == "执行成功":
...
@task
def schedule_for_operate_running(jobName,hostName,script,action,editor,scheduleName,list_id,schedule_id):
    hostName = pc2pcs(hostName)
    if editor == "":
        if action !="":
            time_now = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            ...
            action = 'schedule'
            result = ""
            results.extend([job_name,'blues','手动',time_now,end_time,'unknow','执行开始',job_id,action,result,command,scheduleName,list_id])
