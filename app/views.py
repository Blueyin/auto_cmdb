...
db = MySQLdb.connect(host="127.0.0.1", user="test", passwd="test_1234", db="cmdb", charset="utf8")
db.autocommit(True)
job_enable = 1
work_enable = 1
...
def author_result(request):
    if request.method == 'GET':
        ...
        group = request.GET['group']
        try:
            author_update = Author.objects.filter(id=id).update(name=name, user=user, passwd=passwd, key=key,
                                                                group=group)
            author_update.save()
        except:
            print
...
def macresult(request):
    if request.method == 'GET':
        ...
        idc_bh = request.GET['idc_jg']
        try:
            mac_update = HostList.objects.filter(id=id).update(ip=ip, hostname=name, application=service,
                                                               idc_name=idc_name, bianhao=idc_bh)
            mac_update.save()
        except:
            print
            "get exception"
    ...
    if request.method == 'GET':
        ...
        for i in params:
            ...
            try:
                ...
                print
                "operate id get exception"
            else:
                if operate_id == "":
                    params_add = operate_params(param_id=var_list, param_name=var_name, param_value=var_value,
                    ...
                    try:
                        ...
                        continue
        try:
            action_update = operate_list.objects.filter(id=id).update(name=name, label=label, kinds=kinds, host=host,
    ...
    if request.method == 'GET':
        ...
        for i in params:
            ...
            try:
                ...
                if operate_id == "":
                    params_add = schedule_params(param_id=var_list, param_name=var_name, param_value=var_value,
                                                 param_text=var_text, job_id=id)
                    params_add.save()
                else:
        ...
        try:
            action_update = schedule_for_operate.objects.filter(id=id).update(name=name, label=label, kinds=kinds,
                                                                              host=host, change_time=change_time,
                                                                              script=script, editor=editor,
...
def addaction(request):
    if request.method == 'GET':
        ...
        for i in params:
            ...
            var_text = i['param_text']
            var_params.append(
                operate_params(param_id=var_list, param_name=var_name, param_value=var_value, param_text=var_text,
                               job_id=operate_id))
...
def update_schedule(request):
    if request.method == 'GET':
        ...
        try:
            schedule_update = schedule_list.objects.filter(id=id).update(name=name, code=code, fun=kinds, user=user,
                                                                         editor=editor, action_name=job_id,
                                                                         count_job=len(jobs), change_time=change_time)
...
def add_param(schedule_id, list_id):
    ...
    for j in operate_params.objects.filter(job_id=job_id).values():
        ...
        var_params.append(
            schedule_params(param_id=param_id, param_name=param_name, param_value=param_value, param_text=param_text,
                            job_id=operate_id))
...
def addfun(request):
    if request.method == 'GET':
        ...
        change_time = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))
        try:
            schedule_add = schedule_list(name=name, code=code, fun=kinds, user=user, editor=editor, action_name=job_id,
                                         count_job=len(jobs), change_time=change_time)
            schedule_add.save()
        except:
            print
...
def work_schedule(request):
    ...
    if work_enable == 1:
        ...
        try:
            schedule_running.delay(all_schedule['name'], all_schedule['user'], all_schedule['fun'],
                                   all_schedule['action_name'], id, all_results.values())
            work_enable = 0
        except:
            print
            "编排失败"
...
def soccer_rank(request):
    ...
    # all_result = Soccer_group.objects.all()
    all_result = Soccer_group.objects.all()
